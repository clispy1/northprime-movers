...
const TwoToneIcon = ({ icon: Icon }: { icon: any }) => (
  <div className="relative inline-flex items-center justify-center w-16 h-16 group">
    ...
    <Icon
      size={32}
      className="relative z-10 text-blue-900 transition-transform group-hover:scale-110 duration-300"
    />
  </div>
);
...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-gray-50 font-sans selection:bg-red-200 selection:text-red-900 overflow-x-hidden">
      {/* Top Discount Banner */}
      <div
        className={`fixed top-0 left-0 right-0 z-[60] transition-transform duration-300 ${isScrolled ? "-translate-y-full" : "translate-y-0"} bg-red-600 text-white text-center py-2 px-4 shadow-[0_2px_10px_rgba(220,38,38,0.4)]`}
      >
      ...
      >
        <a
          href="tel:+16044426622"
          className="bg-red-600 text-white font-bold py-4 px-6 rounded-2xl text-lg flex items-center justify-center gap-2 shadow-[0_10px_30px_rgba(220,38,38,0.5)] border border-red-500"
        >
